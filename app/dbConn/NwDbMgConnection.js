(function (context, undefined) {
    ...
    if (typeof module !== "undefined") {
        Class = require('../lib/Nwjsface.js').Class;
    ...
    var NwDbMgConnection = Class(function () {
        ...
        return {
            ...
            getCollectionNames: function (cb) {
                // this.dbConn.getCollectionNames(function (err, result) {
                //     cb(result)
                // })
                this.dbConn.listCollections().toArray(function (err, collections) {
                    cb(collections)
                });
            },
            ...
            backUpChange: function (tableName, findObj, ChangeMethod, callback) {
                this.findOne(tableName, findObj, (doc) => {
                    delete doc._id;
                    doc.change_method = ChangeMethod;
                    this.insert('backUpChange_' + tableName, doc, callback)
                });
            },
            update: function (tableName, findObj, updateObj, callback) {
                var db = this._getDB(tableName);

                this.backUpChange(tableName, findObj, 'update', function (params) {
            ...
            destroy: function (tableName, findObj, callback) {
                var db = this._getDB(tableName);
                this.backUpChange(tableName, findObj, 'remove', function (params) {
